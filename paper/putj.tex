...
However, a few trends can be readily observed from the figure.
First, the models trained with data augmentation (2--5) all generalize at least as well
as the one trained only on clean data (1).
This indicates that the learning algorithm is not corrupted by training on noisy examples.
Between the no-augmentation condition (N) and pitch-shifting augmentation, there is a
small, but consistent improvement in performance.  However, little differentiation can be
observed between different augmentation conditions.
...
\begin{figure}
    ...
    \caption{Point-wise comparison of test-set scores (LRAP)
        ...
        Each point corresponds to a single test track.% and is colored according to
        %the random train-test split from which it was drawn.
\label{onevstwo}}
\end{figure}
...
Finally, we investigate the effects on each class due to data augmentation.
