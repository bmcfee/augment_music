...
\begin{figure*}[t]
    ...
    \caption{Mean label-ranking average precision test-set scores for each 
...
\begin{figure}
    ...
    \caption{Point-wise comparison of test-set scores for augmentation condition 1 (no
        augmentation) vs.\ condition 2 (pitch-shifting).  Each data point corresponds to a
        single test track, and is colored according to the test split index from which it
        was drawn.  The majority of points lie above the main diagonal, and are more 
        accurately predicted by the model trained with data augmentation.\label{onevstwo}}
\end{figure}
\section{Conclusion}
