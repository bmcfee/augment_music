...
then any pixel-wise label annotations (\eg, bounding boxes) should be also be rotated by
that angle.
...
should also result in dilation of time-keyed annotations (\eg, chord or instrument
activation timings)~\cite{mauch2013audio}.

Moreover, many natural, musically-inspired deformation operations
...
Similarly, pitch-shifting a track should induce transpositions of melodic pitch contours,
and if the transposition is sufficiently large, chord labels or symbolic annotations
should also be modified.
It follows that successful application of data augmentation techniques in MIR may require
...
data: if it was easy, the modeling problem would be essentially solved already.
...
data-driven methodology more accessible to the MIR community.
...
and also facilitates task-dependent annotation deformations.  Moreover, we take
...
%
Finally, we borrow familiar software design concepts from the SciKit-Learn
...
parameters --- \eg, multiple time-stretch factors --- or randomly sample from 
a distribution over parameters. 
To support this behavior, deformation objects can implement arbitrary state transition 
logic, and produce a sequence of state objects through Python's \emph{generator} 
functionality.
...
and we encourage the convention that patterns be registered in order of decreasing generality.
...
to pre-compute persistent quantities of interest, such as track duration 
(necessary for target-smearing deformations) or tuning estimations (necessary for 
pitch-shifting deformations).
...
\[
{\Sigma_1 \times \Sigma_2 \times \cdots \times \Sigma_k}.
\]
Consequently, one input example produces $n_p = \prod_{i=1}^k
...
individual deformation objects as optional.  This can be useful when it is
otherwise difficult to encode a special \emph{no-op} state within a deformation object,
such as the randomized time-stretch example listed as \Cref{alg:timestate}.is
...
For this reason, stochastic deformations are designed so that all randomness is contianed
within the state generation logic, and transformations are all deterministic.
...
gradient descent, using mini-batches of $n=64$ patches and a constant learning rate of 0.01.
