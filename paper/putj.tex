...
%       b. big data is hard to come by
%       c. data augmentation has proven useful in vision. 
%   2. but data augmentation is more difficult in music than in images
...
%       b. even if it was, it might not be easy to engineer around them without discarding useful information
%
% alternatively, we can use bigger models, and learn the appropriate invariances from statistics.
...
%   frequencies
%
% 2. we developed a generic, plugin-oriented architecture for doing musical data augmentation
%   a. hooks into JAMS, and inherits validation/schema.  also makes it easy to modify all annotations for a
%   given track in one shot.
%   b. allows the developer to register a deformation against different types of data
%       eg, a ``pitch-shift'' deformer implements an audio deformation, pitch annotation modification, and
%       chord/key manipulators
%   c. modules can be chained or skipped in a pipeline, similar to sklearn feature extractors
%       pipelines can be serialized, stored, shared, and reimplemented easily
%   d. full history of modification state is preserved within the output JAMS sandbox, so the results are
%   documented and reproducible
%       this includes all random state
%   e. deformations can be stochastic, and potentially generate infinite streams of randomized data
...
%       relu
%   layer 2: 48 filters of shape (9x7)
%       relu
