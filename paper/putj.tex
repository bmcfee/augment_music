...
% Title.
% ------
\title{A framework for musical data augmentation}
...
%  {Eric J. Humphrey} {Music and Audio Research Laboratory \\ New York University\\ {\tt
%  ejhumphrey@nyu.edu}}
%  {Juan P. Bello} {Music and Audio Research Laboratory \\ New York University \\ {\tt jpbello@nyu.edu}}
...
\def\authorname{Brian McFee, Eric J. Humphrey, Juan P. Bello}
...
properties.  \Cref{deformer-example} follows with a concrete, but high-level example.
...
%This functionality mimics that of the scikit-learn \emph{Transformer} pipeline,\footnote{\url{http://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html}} except that rather than applying a composition of one-to-one mappings, deformation pipelines compose functions in a one-to-many fashion.
...
\Cref{lrapresults} illustrates the distribution of test-set performance across splits. 
...
\Cref{fscore} depicts the change in $F$-score relative to the baseline
...
artists in MedleyDB, and often produces the fewest examples for training.

The reduction in $F$-score for \emph{synthesizer} in the (PT) condition may 
explain the corresponding reduction in \Cref{lrapresults}, and may be due
...
interface to help train models on distorted data.
The instrument recognition experiment demonstrates that even simple deformations
such as pitch-shifting can improve model generalization, but also demonstrate that some
care should be exercised in the selection of deformations depending on the
characteristics of the problem.  
We note that these results are preliminary, and do not fully exploit the capabilities of the
augmentation framework.
In future work, we will to investigate the effects of data augmentation on a variety of MIR tasks.
